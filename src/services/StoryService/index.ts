...
//   private presetService: PresetService = new PresetService();
...
//     // Configure PresetService base and role defaults
//     try {
//       if (story.basePreset?.source === 'named' && story.basePreset.name) {
//         this.presetService.configureBase({ baseSource: 'named', basePresetName: story.basePreset.name });
//       } else {
//         this.presetService.configureBase({ baseSource: 'current' });
//       }
//       if (story.roleDefaults) {
//         for (const role of Object.keys(story.roleDefaults) as Role[]) {
//           const partial = story.roleDefaults[role];
//           if (partial) this.presetService.registerRoleOverride(role, partial);
//         }
//       }
//     } catch {}
...
//       win: cp.winTriggers,
//       fail: cp.failTriggers,
//     }));

//     if (story.onStart) {
//       this.applyOnActivate(story.onStart);
//     }
...
//     const triggers = this.compiledTriggers[idx];
//     const win = triggers?.win ?? [];
//     const fail = triggers?.fail ?? [];

//     if (fail.some((re) => re.test(text))) {
...
//     // 4) Preset overrides for this checkpoint (register only; apply per role on demand)
//     if (on.preset_overrides && this.story) {
//       const cp = this.story.checkpoints[idx ?? 0];
//       if (cp) {
//         for (const [role, partial] of Object.entries(on.preset_overrides) as [Role, Record<string, any>][]) {
//           if (partial) this.presetService.registerCheckpointOverride(cp.id, role, partial);
//         }
//       }
//     }
//   }
...
//   // Public helper: apply merged preset for a role at the current checkpoint
//   applyRolePreset(role: Role) {
//     if (!this.story) return;
//     const cp = this.story.checkpoints[this.state.currentIndex];
//     if (!cp) return;
//     const label = `(dynamic) ${role} @ ${this.state.title} / ${cp.name}`;
//     this.presetService.applyFor(role, cp.id, label);
//   }
// }
