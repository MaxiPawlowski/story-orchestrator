...
import {
  ...
  TalkControlDraft,
  TalkControlCheckpointDraft,
  TalkControlMemberDraft,
  TalkControlAutoReplyDraft,
  ensureOnActivate,
  cleanupOnActivate,
  splitLines,
  clone,
...
type TabKey = "basics" | "worldinfo" | "automations" | "notes" | "presets" | "transitions" | "talkControl";
...
const TALK_CONTROL_TRIGGER_OPTIONS: Array<{ key: TalkControlTrigger; label: string }> = [
...
const cloneTalkControlMember = (member: TalkControlMemberDraft): TalkControlMemberDraft => ({
...
const cloneTalkControlCheckpoint = (checkpoint?: TalkControlCheckpointDraft): TalkControlCheckpointDraft => ({
...
const CheckpointEditorPanel: React.FC<Props> = ({
  ...
  const selectedCheckpointId = selectedCheckpoint?.id ?? null;
  const checkpointTalkControl = selectedCheckpointId ? talkControl?.checkpoints?.[selectedCheckpointId] : undefined;
                  ...
                  <>
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="text-xs text-slate-300">
                        Configure automated responses for <span className="font-semibold text-slate-100">{selectedCheckpoint.name || selectedCheckpoint.id}</span>.
                      </div>
                                  ...
                                  {member.autoReplies.map((reply, replyIdx) => (
                                    <div key={`reply-${replyIdx}`} className="space-y-2 rounded border border-slate-800 bg-slate-950/60 p-2">
                                      <div className="flex flex-wrap items-center justify-between gap-2">
                                        <div className="flex flex-wrap items-center gap-2 text-[11px] text-slate-300">
                                          ...
                                          <label className="flex items-center gap-1">
                                            ...
                                            <input
                                              type="number"
                                              min={1}
                                              className="w-20 rounded border border-slate-700 bg-slate-800 px-2 py-1 text-sm text-slate-200 focus:border-transparent focus:outline-none focus:ring-slate-600"
                                        ...
                                        <button
                                          type="button"
                                          className="inline-flex items-center justify-center rounded border border-slate-700 bg-slate-800 px-2 py-1 text-xs text-red-300 hover:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-red-500"
